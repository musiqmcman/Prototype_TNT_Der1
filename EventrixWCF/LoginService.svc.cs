    ...
    public class LoginService : ILoginService
    {
        public EventGuest GuestLogin(string email, string password)
        {
            using (EventrixDBDataContext mde = new EventrixDBDataContext())
            {
                try
                {
                    return mde.Guests.Where(pe => pe.Email == email && pe.Password == password).Select(pe => new EventGuest
                    {
                        ID = pe.G_ID,
                        NAME = pe.Name,
                        EMAIL = pe.Email,
        ...
        public EventStaff StaffLogin(string email, string password)
        {
            using (EventrixDBDataContext mde = new EventrixDBDataContext())
            {
                try
                {
                    return mde.Staffs.Where(pe => pe.Email == email && pe.Password == password).Select(pe => new EventStaff
                    {
                        ID = pe.StaffId,
                        NAME = pe.Name,
                        EMAIL = pe.Email,
